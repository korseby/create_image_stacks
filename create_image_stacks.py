...
parser.add_argument('-v', '--version', action='version', version='Create Image Stacks Version 0.2',
                   help='show version')
...
for i, label in enumerate(xmp_labels, start=1):
	if (i > 1) and ( (i >= len(xmp_labels)) or (label != stack_label) or (label == "")):
		...
		# Stack with several files
			...
			if (__DEBUG__ == True): print('mkdir' + ' ' + '\"' + dir_name + '\"')
			if (__DRY_RUN__ == False): os.makedirs(dir_name, parents=True, exist_ok=True)
